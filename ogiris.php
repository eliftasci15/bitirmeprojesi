...
{
	$eposta = $_POST['eposta'];  //fetch records from login form
	$sifre = $_POST['sifre'];
	
	if(!empty($_POST["submit"]))   // if records were not empty
     {
	$loginquery ="SELECT * FROM okullar WHERE eposta='$eposta' && sifre='$sifre'"; //selecting matching records
	$result=mysqli_query($db, $loginquery); //executing
	$row=mysqli_fetch_array($result);
	
	                        if(is_array($row))  // if matching records in the array & if everything is right
								{
                                    	$_SESSION["okulno"] = $row['o_id']; // put user id into temp session
										 header("refresh:1;url=ana2.php"); // redirect to index.php page
	                            } 
							else
							    {
...
body{
	...
	border:none;
	outline:none;
	background-image:url(arkapı.jpg);
...
<label class="girisYazi">Hoş Geldiniz</label>
   ...
   <span style="color:green;"><?php echo $success; ?></span>
    <form action="" method="post">
      <input type="text" id="eposta" placeholder="E-posta"  name="eposta"/>
